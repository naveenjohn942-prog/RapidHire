...
def is_hns(
    ...
    /                        \
    """
    shoulder_height_threshold = avgBarLength * 0.6

    # Head should be higher than both shoulders
    head_higher_than_shoulders = c > max(a, e)

    # Troughs should be lower than both shoulders
    troughs_lower_than_shoulders = max(b, d) < min(a, e)

    # Right end point (f) should be lower than the right shoulder (e)
    right_end_lower_than_right_shoulder = f < e

    # Troughs (b, d) should be roughly at the same level
    troughs_at_same_level = abs(b - d) < avgBarLength

    # Head height should be significantly higher than shoulder height
    significant_head_height = abs(c - e) > shoulder_height_threshold

    return (
        head_higher_than_shoulders
        and troughs_lower_than_shoulders
        and right_end_lower_than_right_shoulder
        and troughs_at_same_level
        and significant_head_height
    )
         ...
         \    /\    D
          \  /  \  /
           \/    \/
            A     C
    """
    # Ensure a and c are lows and b is a significant peak
    price_condition = (
        abs(a - c) <= avgBarLength * 0.5 and  # A and C are close in price
        b > max(a, c) and                    # B is higher than both A and C
        b - min(a, c) > atr * 2              # B is significantly higher than A and C
    )

    # Ensure volume conditions and that D is between B and C
    volume_condition = cVol < aVol and b > d > c
